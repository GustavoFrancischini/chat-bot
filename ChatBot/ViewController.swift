...
class ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
    ...
    // Cor de fundo das respostas do bot.
    private let botResponseBackgroundColor = UIColor.white //
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        if let text = textField.text?.trimmingCharacters(in: .whitespacesAndNewlines), !text.isEmpty {
            ...
            if let response = messageResponses[processedText.lowercased()] {
                messages.append((response, false)) // Marca como resposta do bot
            } else {
                // Se não houve resposta..
                messages.append(("- Ainda não tenho resposta para isso =(", false)) // Marca como resposta do bot
            }
            ...
            // Rolando a tabela para a última mensagem.
